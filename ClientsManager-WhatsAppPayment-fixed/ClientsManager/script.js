...
function markWhatsAppPaymentUsed(phone, amount) {
    const url = getSheetsVerifyUrl();
    const secret = getSheetsVerifySecret();
    if (!url || !secret) {
        return Promise.resolve({ success: false, message: 'إعداد ربط الشيت ناقص. افتح الإعدادات واحفظ VERIFY_SECRET.' });
    }

    return new Promise((resolve) => {
        const callbackName = '__stackSheetsCb_' + Date.now() + '_' + Math.random().toString(36).slice(2);
        ...
        const finish = (result) => {
            ...
            script.remove();
            resolve(result || { success: false, message: 'رد غير صالح من الشيت.' });
        };
        window[callbackName] = finish;
        script.onerror = () => finish({ success: false, message: 'تعذر الاتصال بالشيت. تأكد من رابط Apps Script وأنه منشور كـ Web App.' });
        script.src = url + '?' + params.toString();
        document.head.appendChild(script);
        const timer = setTimeout(() => finish({ success: false, message: 'انتهت مهلة الاتصال بالشيت.' }), 15000);
    });
}
